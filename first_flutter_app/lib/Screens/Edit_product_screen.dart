...
class EditProductScreen extends StatefulWidget {  final String id; 
...
FireStoreService("products"); 
 ...
 final value = await 
firebaseService.getDatabyId(widget.id); 
 nameController.text = value?["name"] ?? "name"; 
 descriptionController.text = value?["description"] ?? "desc"; 
 priceController.text = value?["price"] ?? "price";  debugPrint(value.toString()); 
 setState(() {}); 
 ...
 Padding( 
 padding: const EdgeInsets.all(8.0),  child: TextFormField( 
 controller: descriptionController,  decoration: const InputDecoration(  labelText: "Description", 
 border: OutlineInputBorder(), 
 ), 
 ), 
 ), 
 ...
 onPressed: () async { 
 await firebaseService.updateData(widget.id, {  "name": nameController.text,  "description": descriptionController.text,
